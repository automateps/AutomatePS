        ...
        foreach ($obj in $InputObject) {
            if ($obj.Type -eq "Workflow") {
                ...
                switch ($Connection.Version.Major) {
                    ...
                    {$_ -in 11,22} { $copyObject = [AMWorkflowv11]::new($Name, $Folder, $Connection.Alias) }
                    ...
                    switch ($item.ConstructType) {
                        "Evaluation" {
                            ...
                            switch ($Connection.Version.Major) {
                                ...
                                {$_ -in 11,22} { $newItem = [AMWorkflowConditionv11]::new($Connection.Alias) }
                                default        { throw "Unsupported server major version: $_!" }
                            }
                        ...
                        "Wait" {
                            ...
                            switch ($Connection.Version.Major) {
                                ...
                                {$_ -in 11,22} { $newItem = [AMWorkflowItemv11]::new($Connection.Alias) }
                                default        { throw "Unsupported server major version: $_!" }
                            }
                ...
                foreach ($trigger in $currentObject.Triggers) {
                    switch ($Connection.Version.Major) {
                        ...
                        {$_ -in 11,22} { $newTrigger = [AMWorkflowTriggerv11]::new($Connection.Alias) }
                        default        { throw "Unsupported server major version: $_!" }
                    }
                ...
                foreach ($link in $currentObject.Links) {
                    switch ($Connection.Version.Major) {
                        ...
                        {$_ -in 11,22} { $newLink = [AMWorkflowLinkv11]::new($Connection.Alias) }
                        default        { throw "Unsupported server major version: $_!" }
                    }
                ...
                foreach ($variable in $currentObject.Variables) {
                    switch ($Connection.Version.Major) {
                        ...
                        {$_ -in 11,22} { $newVariable = [AMWorkflowVariablev11]::new($Connection.Alias) }
                        default        { throw "Unsupported server major version: $_!" }
                    }
